...
AddFriends(renew := false, getFC := false) {
    global FriendID, friendIds, waitTime, friendCode, scriptName, deleteMethod
    
    ; NEW: Skip friend addition for training and missions inject methods
...
; Complete Updated CreateAccountList function with injection logic changes
CreateAccountList(instance) {
    ...
    ; ===== INJECTION TYPE PARSING WITH NEW LOGIC =====
    ...
    if (deleteMethod = "Inject") {
        ...
        minPacks := 0
        maxPacks := 38  ; Changed: No max limit, accept anything less than 39P
        LogToFile("Inject mode: range 0-38 (less than 39P)")
    }
    else if (deleteMethod = "Inject Missions") {
        parseInjectType := "Inject Missions"
        minPacks := 0
        maxPacks := 38  ; Changed: No max limit, accept anything less than 39P
        LogToFile("Inject Missions mode: range 0-38 (less than 39P)")
    }
    else if (deleteMethod = "Inject for Reroll") {
        parseInjectType := "Inject for Reroll"
        minPacks := 35  ; Changed: Accept 35P and greater
        maxPacks := 9999  ; Very high number to effectively have no upper limit
        LogToFile("Inject for Reroll mode: range 35-9999 (35P and greater)")
    }
